...
for i in data_places:
    ...
    plot1 = axarray[0]
    plot2 = axarray[1]
    plot1.plot(data[0], data[i] ,label = "{}".format(filenames[data_places.index(i)].split(".")[0]))

    
    
    #plot1.legend(loc = 3)
    #plot1.title("Evolution of eccentricity of {}".format(filenames[data_places.index(i)].split(".")[0]))
    plot2.plot(data[0], data[i-1] ,label = "{}".format(filenames[data_places.index(i)].split(".")[0]))

    #plt.xlabel("Time (Yrs)")
    plot2.set_ylabel("Inclination ($^o$)")
    plot1.set_ylabel("Eccentricity")
    plot2.set_xlabel("Time (Yrs)")
    plot1.legend(loc = 1)
    fig.suptitle("Evolution of inclination and eccentricity")
    fig.savefig("ei{}.png".format(filenames[data_places.index(i)].split(".")[0]))

'''
#Plotting the orbits
data_places = list(range(2,24,7))
data_places1 = list(range(2,52,7))
